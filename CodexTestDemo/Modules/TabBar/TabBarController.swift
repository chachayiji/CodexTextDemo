...
final class TabBarController: UITabBarController {
  ...
  private func configureTabs() {
    ...
    let newsNav = BaseNavigationController(rootViewController: newsController)
    newsNav.tabBarItem = UITabBarItem(title: "新闻", image: UIImage(systemName: "newspaper"), tag: 0)

    let videoController = VideoViewController()
    let videoNav = BaseNavigationController(rootViewController: videoController)
    ...
    let mallNav = BaseNavigationController(rootViewController: mallController)
    mallNav.tabBarItem = UITabBarItem(title: "商城", image: UIImage(systemName: "cart"), tag: 2)

    let profileController = ProfileViewController()
    let profileNav = BaseNavigationController(rootViewController: profileController)
